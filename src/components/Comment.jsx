...
import { scoreComment } from "@/utils/toxicityClient"
...
export default function Comment({ id }) {
  ...
  async function handleSubmit(e) {
    ...
    let scores;
    try {
      scores = await scoreComment(comment);
    } catch (err) {
      console.error(err);
      alert("Could not verify your comment toxicity. Please try again")
    }

    if (scores.toxic_flag || scores.obscene_flag || scores.insult_flag || scores.severe_toxic_flag || scores.threat_flag) {
      alert("Your comment looks too toxic-please follow community guidlines")
      return;
    }
